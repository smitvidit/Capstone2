...
public class CheckoutShippingPage {
	...
	private final By newAddress = By
			.xpath("/html/body/div[2]/main/div[2]/div/div[3]/div[4]/ol/li[1]/div[2]/div[2]/button/span");

	private final By shipHere = By.xpath("/html/body/div[3]/aside[2]/div[2]/footer/button[1]/span");
	...
	private final By state = By
			.xpath("/html[1]/body[1]/div[4]/aside[2]/div[2]/div[1]/div[1]/form[1]/div[1]/div[5]/div[1]/select[1]");
	...
	// private final By itemsInCart =
	// By.xpath("//*[@class='title']//*[contains(text(), '2')]");
	private final By itemsInCart = By.xpath("//*[@id=\"opc-sidebar\"]/div[1]/div/div[1]/strong/span[1]");

	private final By productOneName = By
			.xpath("(//*[@class='product-item-details'])[1]//*[@class='product-item-name']");
	...
	private final By productTwoName = By
			.xpath("(//*[@class='product-item-details'])[2]//*[@class='product-item-name']");
	...
	public boolean checkCountryIsListed(String optionText) {
		WebElement selectCountryFromDropDown = wait.until(ExpectedConditions.visibilityOfElementLocated(country));
		Select select = new Select(selectCountryFromDropDown);
	...
	public boolean isNewAddressButtonPresent() {
		...
		try {
			// Find the element using its XPath (you can use other locators like ID, class
			// name, etc.)
			WebElement newaddress = driver.findElement(newAddress); // Change the XPath accordingly

			// If the element is found, print a success message
			System.out.println("newaddress is present on the page.");

		} catch (NoSuchElementException e) {
			// If the element is not found, print a failure message
			System.out.println("newaddress is not present on the page.");
			return false;
		}
		return true;
	...
	public void selectCountry(String countryMKD) throws InterruptedException {
		WebElement selectCountryFromDropDown = wait.until(ExpectedConditions.visibilityOfElementLocated(country));
		Select select = new Select(selectCountryFromDropDown);
		select.selectByVisibleText(countryMKD);

		Thread.sleep(500);
		selectCountryFromDropDown.sendKeys(Keys.ENTER);
	}

	public void selectState(String State) throws InterruptedException {
		WebElement selectStateFromDropDown = wait.until(ExpectedConditions.elementToBeClickable(state));
	...
	public void clickNewAddressButton() {
		WebElement clickNewAddressButtonWE = wait.until(ExpectedConditions.visibilityOfElementLocated(newAddress));
		clickNewAddressButtonWE.click();
	}

	public void clickShipHereButton() {
		WebElement clickShipHereButtonWE = wait.until(ExpectedConditions.elementToBeClickable(shipHere));
		clickShipHereButtonWE.click();
	}
	...
	// I have implemented a Retry Mechanism where I try to click "Place Order"
	// button multiple times before declaring a failure.
	// I did this because the element is often not clickable due to transient
	// issues.
	public void clickOrderSummaryLink(int maxRetries) {
		for (int retry = 0; retry < maxRetries; retry++) {
			try {
				WebElement orderSummaryLink1 = wait.until(ExpectedConditions.elementToBeClickable(orderSummaryLink));
		...
		// If the loop completes without a successful click, you can throw an exception
		// or log an error.
	}

	public String getProductOneName() throws InterruptedException {
